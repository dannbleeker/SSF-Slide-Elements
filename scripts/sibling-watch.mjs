...
export const TRIAGED = {
  ...
  "charts:issue:6658":
    'RELEVANT, and it is about the SUBMISSION rather than the runtime — triaged 2026-09-16. There is no manifest element that declares which HOSTS an add-in supports: Partner Center derives the Packages list from `<Requirements>`, and narrowing that block to force a host out fails package validation outright with "No Supported Office Products". The exposure here is the MIRROR of the sibling\'s, which wanted PowerPoint on the web off its list: this add-in intends to support all four hosts and has measured two, Mac and iPad having no answer sheet (`docs/DESIGN.md` section 9) and the validators\' report being the first measurement for both. This manifest carries NO `<Requirements>` at all, deliberately — the floor is checked at runtime by `checkFloor` — so the Packages tab will list every host PowerPoint runs on, which is what is wanted and also what leaves no lever if a validator finds a defect on one of the two unmeasured ones. THE TRAP THIS ROW EXISTS TO PREVENT: the obvious response to such a report is "add a `<Requirements>` block so iPad drops off the list", and that is precisely the workaround this issue records FAILING — it would turn one host\'s defect into a package with no supported products at all. The answer is to fix it, or to withdraw that host in Partner Center; never a `<Requirements>` edit.',
  "charts:issue:3784":
    'RELEVANT, and the reason changed under it — re-verdicted 2026-09-12. Shape TAGS are lost when a shape is cut and pasted on PowerPoint web. This row used to say nothing written onto a shape may be something a later step depends on finding, and since #48 two shipped features depend on exactly that: "Used in this deck" and "Remove from N slides" both find their shapes through `readShapeTags` (`docs/DESIGN.md` section 6). So the exposure is real and is DOCUMENTED rather than guarded, which is the same decision both siblings took and the same one `docs/MANUAL.md` carries: a cut-and-pasted element stops being listed and stops being removable, and no read can tell that from a shape the user drew. Not detectable, so not guarded.',
  "charts:issue:6266":
  ...
  "charts:question:scratch-slides-returned":
    "ADOPTED as doctrine (CLAUDE.md host rules), and in code since 2026-09-10 — a sibling's by-id clean-up left 45 blank slides; any removal here is positional and clamped; re-triaged 2026-09-11 against the shipped insert and against 'Remove from N slides'. That one can leave a deck half-changed by design rather than by accident: it stops at the first cycle the deck's own size does not confirm and reports how far it got, which is the opposite failure to 45 slides nobody asked for. Read again on 2026-09-12 against the shared-part carry of #54, which gives it a second edge: a picture is now carried ONCE and pointed at by every later insert of the same element, so a removal that swept what its shapes referenced would take a part other elements are still using. It sweeps nothing — the tag parts and pictures are left as orphans by design (`docs/DESIGN.md` section 6) — and that decision, made before the parts were shared, is what keeps this safe now that they are.",
  "charts:question:shapes-by-index-vs-items":
    'RELEVANT, and the row before this one was wrong — corrected 2026-09-12. It said "a collection this add-in never loads", and `selectedShape` in `src/office/powerpoint.ts` loads one: `getSelectedShapes()` then `load("items/left,items/top,items/width,items/height")`, for an element that lands at the cursor. What saves it is not that the read does not happen but that only `items[0]` is used and a selection is rarely near the ~50 the ceiling is about: a short read that still carries item 0 is the same answer, and one that carries none is the documented fallback, the slide centre (`docs/DESIGN.md` section 5). Most shape reads here are still out of the PACKAGE — the harvest, the removal, the preview card\u2019s grey boxes — and none of those go near `slide.shapes`.',
  ...
  "charts:question:shapes-items-count-honest":
    "RELEVANT, bounded — corrected 2026-09-12 with `shapes-by-index-vs-items`, which this row inherited its false premise from. The one shape collection this add-in loads is `getSelectedShapes` in `selectedShape`, and it reads `items[0]` alone, so an honest count is not something any decision here turns on. The SLIDE collection is where a short read costs a wrong index, and that is `issue:4272`'s.",
  "charts:question:shapes-items-via-positional-slide":
    "RELEVANT, bounded — see `shapes-items-count-honest`, corrected with it on 2026-09-12. Reaching a shape collection through a positionally-read slide is not what `selectedShape` does — it asks the presentation for the selection — so this row's exact shape is still untouched, and it is no longer true that no shape collection is loaded at all.",
  "charts:question:tags-add-same-key-twice":
    'NO EXPOSURE to the API question. The package equivalent is real: an insert writes TWO keys onto the shapes it lands — `SSF_SLIDE_ELEMENT` and `SSF_SLIDE_ELEMENTS_CATALOGUE` (`src/core/splice/splice.ts`) — into the next free `tagN` rather than over `tag1.xml`, which would destroy another tool\'s tags (`src/core/pptx/tags.ts`). Corrected 2026-09-12: this row said "it writes one now" while `issue:6079` said two on the same day, and two is right.',
