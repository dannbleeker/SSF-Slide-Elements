...
| Stamping several slides at once — select several slides and a stamp lands on every one of them, one cycle each, the pane's Undo disarmed and the footer saying so | done — not yet run on a real PowerPoint; `selectedSlides`, the read behind it, has never been asked of one for more than its first item |
