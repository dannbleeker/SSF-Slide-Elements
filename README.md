...
| Host probe — a Script Lab snippet that asks a real PowerPoint the eight questions the design rests on (a pruned package on the way in, insert-then-delete order, the selected slide, which read of the deck, Ctrl+Z, timing and the floor, moving to a slide, and what the slide listing says about a slide just inserted), and a reader that says what each answer means and files the sheet | done — thirteen sheets under `docs/host-answers/`: seven from PowerPoint for the web (four 2026-09-10, two 2026-09-14, one 2026-09-24) and six from Windows (two 2026-09-11, two 2026-09-14, two 2026-09-24). **All eight questions are answered on both platforms.** Question 8, added 2026-09-24 for the Undo, answered yes on both: the listing carries the creation id the package brought, straight away and still later, and a creation id is unique in it |
...
| The picker — browse by section, search (with the matched words highlighted), insert, Recent, favourites, Undo one deep | done |
| Undo that refuses rather than guess — it checks the deck is the size the insert left it, that your own slide is not already back, and that the slide it inserted is still where it put it; a drag, a deletion or a duplicate all get a refusal naming what it saw and nothing deleted | done — the last of those keys on the creation id the engine writes, which probe question 8 measured both hosts handing back on 2026-09-24. **Run on PowerPoint for Windows and for the web the same day**: every refusal fired on both, the deck came out identical each time, the control restored the user's own slide, and an answer takes 11–59 ms. On a host that names no creation id it falls back to the count-checked positional Undo |
| Used in this deck — which library elements the open deck already holds, and where, read from the tags an insert writes | done |
| The jump — a slide number in Used in this deck goes to that slide, and the pane says so only once PowerPoint reports being there | done — read back on every click. Probe question 7 has now measured the call on both platforms this repo has sheets for: the web on 2026-09-14 (998 ms and 1,146 ms twelve minutes apart, the selection put back, the next read answering in 590 ms and 567 ms) and Windows the same day (7 ms and 8 ms, the next read in 58 ms and 2 ms). Borrowed from SSF-Charts on Mac and iPad, which have no sheet |
| Right-click a tile for the other insert target, for one insert | done |
| Stamping several slides at once — select several slides and a stamp lands on every one of them, one cycle each, the pane's Undo disarmed and the footer saying so | done — **run on PowerPoint for Windows, 2026-09-24**: a stamp across 60 selected slides, every slide gaining exactly one shape and all 60 labels surviving, read back from the deck. `selectedSlides` has now been asked for sixty items and answered sixty |
