...
| A grouped insert stamps the shapes INSIDE the group as well as the group, because ungrouping is one gesture and it destroys the group and its tag together — measured: a five-shape element ungrouped went from one use to not in the deck at all. The reader stops at a tagged shape, so while it is a group the answer is unchanged; a shape with no `<p:nvPr>` is skipped rather than refused, since failing an insert that works today is the worse trade. About 250 bytes a shape | decided in the build, 2026-09-11 |
