...
| A run that cannot confirm its insert asks the IDS whether the copy is in the deck, instead of assuming it is not. The confirmation is a COUNT, and one failed count covers two decks that need opposite sentences: an insert that never landed, and one that landed while something else moved the count back. The second leaves a copy, and the bare `break` there reported "The rest are as they were" over it — measured on Windows on 2026-09-24, a deck holding one slide twice with the stamp on the copy. `copyLanded` answers from the creation id the engine wrote, in three states rather than two, because treating "could not confirm" as "did not happen" is the defect itself; and on a host that marks no ids the pane now says it could not tell rather than choosing the nicer guess | measured, 2026-09-24 |
...
| The validators' test deck is SERVED from the site (`public/validators.pptx`, held byte for byte to `template/validators.pptx`) and the testing notes link it, rather than being attached to the submission. Partner Center's notes-for-certification field takes text and nothing else, so the "the test deck attached to this submission" the notes carried for a fortnight pointed at a file that could never have been there. The site is where the notes already send a validator — the privacy and support pages — so the deck goes on the same origin rather than a file host or the repository, which Microsoft's submission rules forbid a listing URL from pointing at | 2026-09-24, on the submission being blocked |
