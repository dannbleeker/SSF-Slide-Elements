...
| A picture cannot be inserted while a shape is selected, and `setSelectedShapes([])` may never resolve (office-js#3698) | SSF-Charts' selection ladder | **Relevant as the same class.** `setSelectedShapes` itself is never made here; whether a slide insert is safe with a standing selection is what SSF-Merge's probe asks, unanswered as of 2026-09-08. |
| `setSelectedSlides` does NOT wedge the web host: SSF-Charts calls it in shipped code (`showSlide`) and its selection ladder ran it in every archived round, 2,429 rungs over about 347 rounds between 2026-08-13 and 2026-09-04, none silent, none refused; the one wedge that family measured was `setSelectedShapes([id])` on build `55011a3` | SSF-Charts' `docs/ROUND-LOOP-JOURNAL.md` and `rounds/*.json`, read 2026-09-12 | **Adopted, for the web, with a read-back.** The jump in "Used in this deck" (2026-09-12) is the one place this add-in writes a selection; `src/host/jump.ts` claims a jump only when the selection read back names the slide. Not measured by that sibling or by this repo on Windows or Mac (office-js#3552 says desktop throws while the notes pane has focus); probe question 7 is the arm that measures it, and re-verdicts this row when a sheet answers. |
...
| No supported way to declare which HOSTS an add-in supports; Partner Center derives the Packages list from `<Requirements>`, and narrowing that block to force a host out fails validation with "No Supported Office Products" (office-js#6658, filed by SSF-Charts) | SSF-Charts' `KNOWN_ISSUES` | **Relevant to the submission, triaged 2026-09-16.** The mirror of the sibling's case: it wanted PowerPoint on the web off its list, while this add-in intends all four hosts and has measured two. This manifest carries no `<Requirements>` by design, so the Packages tab lists every host — wanted, and also no lever if a validator finds a Mac or iPad defect. **Never reach for a `<Requirements>` block to drop a host**: that is the workaround this issue records failing, and it turns one host's defect into a package with no supported products. Fix it, or withdraw the host in Partner Center. |
