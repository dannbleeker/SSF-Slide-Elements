...
| `src/pane/` | `steps.ts` (which step, what the one button says, why it is blocked), `render.ts` (the DOM), `catalogue.ts` (the index and an element's markup, fetched from the site), `storage.ts` (what the pane remembers and in which of its two buckets), `main.ts` (**the only file here allowed to touch Office.js**), plus the HTML and the SSF stylesheet |
